...
from final.json_extended import ExtendedJSONEncoder, ExtendedJSONDecoder
...
class Room(OnlineUserMixin, models.Model):
    ...
    # 한글 채팅방 이름 필드로서 사용
    # name 필드에서는 유일성 체크를 하지 않으므로 같은 이름의 채팅방도 만들 수 있음
...
def room__on_post_delete(instance: Room, **kwargs):
    # Consumer Instance 밖에서 채팅방 그룹에 속한 모든 Cosumer Instances 들에게 chat.room.deleted 메시지를 보냄
